...
| 국내 금리 | 한국은행 ECOS API | 자동 (매시간, 키 필요) |
| 주유소 기름값 | 오피넷(한국석유공사) API | 자동 (매시간, 키 필요) |
...
`scripts/fetch-rates.mjs`(한국은행 ECOS)와 `scripts/fetch-oil.mjs`(오피넷)는
각각 `ECOS_KEY`·`OPINET_KEY` 환경변수로 인증키를 받습니다. **GitHub Actions
Secret 으로만 전달하며 코드·데이터에 절대 넣지 마세요.**

- 금리: 기준금리·정기예금·콜금리·국고채 3년/10년. 통계·항목 코드는 스크립트
  상단 `SERIES` 배열에 있습니다 (ECOS StatisticSearch API 로 실검증).
- 기름값: 전국 주유소 평균 판매가(휘발유·경유·고급휘발유·등유·LPG). 오피넷
  `avgAllPrice.do` 응답의 제품코드→표기명 매핑은 `PRODUCTS` 에 있습니다.
- 금리는 화면의 `지표` 메뉴, 기름값은 `유가·원자재` 메뉴에 섹션으로 붙습니다.
- 금리 변화는 **%p(퍼센트포인트)** 로만 표기합니다. 비율 변화율(예: 2.5%→2.75%
  를 +10%)은 오해를 부르므로 쓰지 않습니다.

### 로또 번호 추천에 대하여

`scripts/lotto-picks.mjs` 가 매주 다음 회차용 추천 5게임을 생성하고,
추첨이 끝나면 실제 당첨번호와 대조해 등수를 `data/lotto-picks.json` 에
기록합니다. 화면(로또 페이지)에는 이번 주 추천, 즉석 생성 버튼, 지난
성적 표가 표시됩니다.

- **로또는 완전 무작위라 당첨 확률을 높이는 방법은 없습니다.** 추천은
  과거 당첨 조합의 통계 패턴(출현 빈도 가중 + 합계 100~175·홀짝 2~4·
  고저 2~4·연속 3개 배제 필터)을 닮은 조합일 뿐입니다. 화면에도 그렇게
  고지합니다.
- 추천은 회차당 한 번만 생성하고 저장해 주중 고정됩니다(매시간 재생성 안 함).
- 클라이언트 '직접 뽑기' 는 같은 알고리즘의 즉석 생성이며 기록되지 않습니다.
